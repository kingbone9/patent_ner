...
def predict():
    ...
    processor.process()
    # 加载数据
    data = np.load(config.predict_dir, allow_pickle=True)
    word_predict = data["words"]
    label_predict = data["labels"]
    predict_dataset = NERDataset(word_predict, label_predict, config)
    logging.info("--------Dataset Build!--------")
    # build data_loader
    predict_loader = DataLoader(predict_dataset, batch_size=config.batch_size,
                                shuffle=False, collate_fn=predict_dataset.collate_fn)
    logging.info("--------Get Data-loader!--------")
    # Prepare model
    if config.model_dir is not None:
        model = BertNER.from_pretrained(config.model_dir)
        ...
        logging.info("--------No model to test !--------")
        return
    val_metrics = evaluate(predict_loader, model, mode='predict')
